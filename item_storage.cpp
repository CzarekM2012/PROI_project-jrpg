...
{
    int i=0;
    while(i<80 && equipment_[i]!=equipment){i++;}
    if(i<80){return i;}
    return -1;
}

consumable *item_storage::get_consumable(int index){return consumables_[index].get_id() != -1 ? &consumables_[index] : nullptr;}

int item_storage::get_consumable_slot(entity *consumable)
{
    int id = consumable->get_id(), i=0;
    while(i<20 && consumables_[i].get_id() != id){i++;}
    if(i<20){return i;}
    return -1;
}

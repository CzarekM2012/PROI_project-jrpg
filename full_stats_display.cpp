...
void full_stats_display::display_character(pc* pc)
{
    ...
    QString name = pc == nullptr ? "" : represented_character_->get_name();
    name_->setText(name);
    update_stats();
    emit lock_equip();
}

void full_stats_display::display_item(entity* to_use)
{
    if(represented_character_ != nullptr)
    {
        int pc_stats[9], new_pc_stats[9];
        represented_character_->get_stats(pc_stats);
        if(to_use->get_counter() == 0){if(!represented_character_->calculate_stats_after_equip(to_use, new_pc_stats)){return;}}
        else{if(!represented_character_->calculate_stats_after_consuming(to_use, new_pc_stats)){return;}}
...
void full_stats_display::clear()
{
    represented_character_ = nullptr;
    name_->setText("");
    for(int i=0; i<7; i++){stat_[i]->label_->setText("");}
}

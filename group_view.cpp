...
group_view::group_view():switchable_frame()
{
    for(int r=0; r<2; r++)
    {
        for(int p=0; p<2; p++)
    ...
    for(int r=0; r<2; r++)
      {
        for(int p=0;p<2;p++)
          {
            if(formation_[r][p]->get_represented_object()->valid())
              {
                if(formation_[r][p]->get_represented_object()->alive())
                  {formation_[r][p]->set_standard_graphic();}
                else{formation_[r][p]->set_standard_graphic("-dead");}
              }
          }

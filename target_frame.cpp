...
void target_frame::set_standard_graphic(const char *type_of_graphic)
{
    ...
    {
        QString name = represented_object_->get_name();
        setStyleSheet(QString("QFrame{border-image:url(:/images/") + represented_object_->get_name() + QString(type_of_graphic) + QString(".png)}"));
        return;
    }
    setStyleSheet("");
}
